...
def beam_search(enc_hid, enc_out, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, model, start_id, end_id,
                unk_id):
    batch_size = len(enc_hid[0])
    beam_idx = T.LongTensor(list(range(batch_size)))
    beams = [Beam(start_id, end_id, unk_id, (enc_hid[0][i], enc_hid[1][i]), ct_e[i]) for i in
             range(batch_size)]  # For each example in batch, create Beam object
    ...
    for t in range(config.max_dec_steps):
        # remaining(rem),beam
        # (rem*beam,)
        x_t = T.stack([beam.get_current_state() for beam in beams if beam.done == False]).contiguous().view(-1)
        # rem*beam, n_emb
        x_t = model.embeds(x_t)
        # rem*beam,n_hid
        dec_h = T.stack([beam.hid_h for beam in beams if beam.done == False]).contiguous().view(-1, config.hidden_dim)
        # rem,beam,n_hid
        # rem*beam,n_hid
        dec_c = T.stack([beam.hid_c for beam in beams if beam.done == False]).contiguous().view(-1, config.hidden_dim)
        # rem,beam,n_hid
        # rem,beam,n_hid
        ct_e = T.stack([beam.context for beam in beams if beam.done == False]).contiguous().view(-1,
                                                                                                 2 * config.hidden_dim)
        if sum_temporal_srcs is not None:
            # rem*beam, n_seq
            sum_temporal_srcs = T.stack(
                [beam.sum_temporal_srcs for beam in beams if beam.done == False]).contiguous().view(-1, enc_out.size(1))

        if prev_s is not None:
            # rem*beam, t-1, n_hid
            prev_s = T.stack([beam.prev_s for beam in beams if beam.done == False]).contiguous().view(-1, t,
                                                                                                      config.hidden_dim)
        s_t = (dec_h, dec_c)
        enc_out_beam = enc_out[beam_idx].view(n_rem, -1).repeat(1, config.beam_size).view(-1, enc_out.size(1),
                                                                                          enc_out.size(2))
